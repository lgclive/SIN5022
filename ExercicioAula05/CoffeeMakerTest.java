...
public class CoffeeMakerTest {
    ...
    /*
        Não é possível testar todas as branches pois:
         1. CoffeeMaker: a quantidade de dinheito utilizada para fazer uma receita não pode ser negativa E maior que 500 ao mesmo tempo;
         2. Inventory: os métodos "set" da classe Inventory são acessados apenas pelo construtor da própria classe
     */
    @Test
    public void testMakeRecipeNegativeAmtMoney() throws InvalidValueException, InsufficientAmountOfMoneyException, RecipeException, InventoryException, AmountOfRecipeException, DuplicatedRecipeException{
        boolean ok = CM.addRecipe(r4);
        assertThrows(InvalidValueException.class, () -> CM.makeCoffee("Mix", -1));
    }

    @Test
    public void testMakeRecipeOverLimitAmtMoney() throws InvalidValueException, InsufficientAmountOfMoneyException, RecipeException, InventoryException, AmountOfRecipeException, DuplicatedRecipeException{
        boolean ok = CM.addRecipe(r4);
        assertThrows(InvalidValueException.class, () -> CM.makeCoffee("Mix", 501));
    }

    @Test
    public void testAddUnknownNameRecipe() throws AmountOfRecipeException, DuplicatedRecipeException{
        boolean ok = CM.addRecipe(r1);
        assertTrue(ok);
        assertThrows(RecipeException.class, ()-> CM.makeCoffee("Water", 10));
    }

    @Test
    public void testAddInvalidTwoRecipesAllSimilarIgredients() throws AmountOfRecipeException, DuplicatedRecipeException, InvalidValueException{
        r1.setAmtCoffee(21);
        boolean ok = CM.addRecipe(r1);
        assertTrue(ok);
        assertThrows(AmountOfRecipeException.class, ()-> CM.makeCoffee("Coffee", 50));
    }
